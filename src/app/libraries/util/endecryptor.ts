...
export class Endecryptor {
  ...
  constructor(
    ...
    decryption_valid_TS_interval: number = 60
  ) {
  ...
  encrypt(message: string) {
    var encryptor = crypto.createCipheriv(
      this.method,
      this.secret,
      this.temp_iv
    );
    this.temp_encrypted =
      Buffer.from(this.temp_iv).toString('base64') +
      encryptor.update(message, 'utf8', 'base64') +
      encryptor.final('base64');
    this.temp_hmac = crypto
      .createHmac('md5', this.secret)
      .update(this.temp_encrypted)
      .digest('hex');
    this.temp_iv = crypto.randomBytes(16).toString('hex').substr(0, 16);
  }
  ...
  decryptAndValidateTS(message: string, hmac: string) {
    if (this.decrypt(message, hmac)) {
      ...
      var messageTS = new Date(
        this.temp_decrypted.substr(0, 19) + 'Z'
      ).getTime();
      if (
        Math.round((currentTS - messageTS) / 1000) <= this.valid_TS_interval
      ) {
